  ...
  dev: {
    ...
    proxyTable: {
      '*': {//192.168.95.74:9003
        ...
        target: 'http://hzfm.beyondsofthz.com',// 你请求的第三方接口娄靖龙ip：192.168.91.27 31
      ...
      '/h5': {//192.168.95.74:9003
        ...
        changeOrigin: true,////在本地会创建一个虚拟服务端，然后发送请求的数据，并同时接收请求的数据，这样服务端和服务端进行数据的交互就不会有跨域问题
        pathRewrite: {
          '/h5': ''// 替换target中的请求地址，也就是说以后你在请求http://api.jisuapi.com/XXXXX这个地址的时候直接写成/api即可。
        }
      }
    },
    // Various Dev Server settings
    host: 'localhost', // can be overwrittengit by process.env.HOST
