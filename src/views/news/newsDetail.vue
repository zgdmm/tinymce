<template>
    <div>
        <div style="padding:20px" class="detail">
            <div class="ats-sale-title" @click="$router.push('/news')">行业动态管理</div>
            <div style="background:#fff;padding:20px">
                <div style="width: 100%;">
                    <p class="title">行业动态详情</p>
                    <!--新增经销商-->
                    <el-form :label-position="labelPosition" ref="form" :model="dataForm"  label-width="120px">
                        <el-form-item label="行业动态标题" prop="articleTitle">
                            <!--<el-input v-model="dataForm.articleTitle"></el-input>-->
                            <span>{{!$route.query.id ? dataForm.articleTitle: dataForm.title}}</span>
                        </el-form-item>
                        <el-form-item label="发布时间" prop="publishTime">
                            <span>{{!$route.query.id?dataForm.publishTime:dataForm.addDateStr}}</span>
                            <!--<el-date-picker type="date" placeholder="选择日期" value-format="yyyy年MM月dd日"  v-model="dataForm.publishTime" style="width: 100%;"></el-date-picker>-->
                        </el-form-item>
                        <el-form-item label="行业动态内容" prop="articleContent">
                            <div v-html="!$route.query.id?(dataForm.articleContent||'--'):dataForm.content"></div>
                            <!--<editor id='tinymce' v-model='dataForm.articleContent' :init='init'></editor>-->
                        </el-form-item>
                        <!--<el-form-item label="图片" prop="articleImgs">
                            <img :src="!$route.query.id?dataForm.articleImgs:dataForm.titleImage" style="width:189px"/>
...
export default {
    ...
    methods: {
        ...
        queryProduct(code) {
            const self = this;
            let param = { articleCode: code };
            let url = '/newsMng/queryNewsInfoByCode';
            if(this.$route.query.id!=''){
                param = { id: this.$route.query.id };
                url = '/news/getHtmlNewsDetail';
            }
            self.$http.post(url,param, result => {
                if (result.code) {
                    ...
                    self.dataForm = result.data;
                    self.dataForm.articleContent = result.data.articleContent||'--';
                    self.dataForm.articleImgs=result.data.imgsList&&result.data.imgsList[0]&&result.data.imgsList[0].url||'';
                    console.log(self.dataForm.articleImgs,'lll');
                } else {
                    self.$message({
        ...
        addAccount() {
            if(this.dataForm.articleImgs.indexOf('http')>-1){
                this.dataForm.articleImgs = (this.dataForm.articleImgs.split('/')[6])
            }
            ...
            if (this.articleCode) {
                ...
                this.$http.post('/newsMng/updateNewsInfo', params, response => {
                    if (response.success) {
                        this.$router.push({ path: 'news' });
                    }
                    this.$message({
