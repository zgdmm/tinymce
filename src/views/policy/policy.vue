<template>
  <div id="product-manage" style="padding: 20px">
    <div class="ats-sale-title">政策法规管理</div>
    <div class="ats-sale-main">
      <div>
        ...
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column label="政策法规标题" min-width="150" prop="productType" show-overflow-tooltip>
          ...
          <el-table-column label="发布时间" prop="productModel">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" prop="operation" min-width="160">
            <template slot-scope="scope">
              ...
              <el-button round type="text" @click="insert(scope.row)" size="small">导入微信政策法规
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block" style="text-align:right;margin-top:18px">
          <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="params.pageNo" :page-sizes="[10, 20, 30, 40]" :page-size="params.pageSize" layout=" sizes, prev, pager, next, jumper" :total="totalCount">
          </el-pagination>
        </div>
      </div>
      <div class="txt-right" style="margin-top:20px">
        <el-input style="width:160px" placeholder="输入关键字找一找" prefix-icon="el-icon-search" v-model="articleTitle" @change="setTitle" @keyup.enter.native="searchBot(1)">
        </el-input>
      </div>
      <div>
        <div>微信政策法规</div>
        ...
        <el-table :data="botList" style="width: 100%">
          ...
          <el-table-column label="政策法规标题" min-width="150" prop="articleTitle" show-overflow-tooltip>
            <template slot-scope="scope">
          ...
          <!--<el-table-column label="政策法规内容" min-width="150"  prop="articleContent" show-overflow-tooltip>
              <template slot-scope="scope">
          ...
          <el-table-column label="操作" prop="operation" min-width="160">
            <template slot-scope="scope">
              ...
              <el-button round type="text" v-if="scope.row.isImport==0" @click="send(scope.row)" size="small">推送
              </el-button>
        ...
        <div class="block" style="text-align:right;margin-top:18px">
          <el-pagination background @size-change="handleSizeChangeBot" @current-change="handleCurrentChangeBot" :current-page="params.botNo" :page-sizes="[10, 20, 30, 40]" :page-size="params.botSize" layout=" sizes, prev, pager, next, jumper" :total="totalCountBot">
          </el-pagination>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    insert(row) {
      let params = {};
      let url = '/policy/getHtmlPolicyDetail';
      ...
      this.$http.post(url, { id: row.id }, result => {
        if (result.code) {
          ...
          params.articleImgs = result.data.titleImage && result.data.titleImage.split('//')[2];
          params.isImport = 1;
    ...
    send(row) {
      var self = this;
      self.$http.post('/policy/getHtmlPolicyList',
    ...
    search(page) {//获取设备列表
      ...
      var self = this;
      self.$http.post('/policy/getHtmlPolicyList',
        { startPosition: ((this.params.pageNo - 1) * this.params.pageSize), maxLength: this.params.pageSize, type: 10 }, result => {
          if (result.code) {
    ...
    searchBot(page) {
      ...
      self.$http.post('/policyMng/queryPolicyInfoByCondition', { pageStart: ((this.params.botNo - 1) * this.params.botSize), pageSize: this.params.botSize, articleTitle: this.articleTitle || '' }, result => {
        // console.log(result);
        if (result.code) {
    ...
    delItem(articleCode) {
      var self = this;
      self.$http.post('/policyMng/deletePolicyInfo', { "articleCode": articleCode }, result => {
        if (result.code) {
          self.$message({
            message: result.message,
            type: 'success'
