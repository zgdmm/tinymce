<template>
  <div id="product-manage" style="padding: 20px">
    <div class="ats-sale-title">通知公告管理</div>
    <div class="ats-sale-main">
      <div class="txt-right">
        <el-input style="width:160px"
          placeholder="输入关键字找一找"
          prefix-icon="el-icon-search"
          v-model="articleTitle" @keyup.enter.native="searchBot(1)" @change="setTitle">
        </el-input>
      </div>
      <div>
        <!--<div>pc端通知公告列表</div>
        ...
        <!--<el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column label="通知公告标题" min-width="150" prop="productType">
      ...
      <div>
        ...
        <el-table :data="botList" style="width: 100%">
          <el-table-column label="通知公告标题" min-width="150" prop="articleTitle" show-overflow-tooltip>
            <template slot-scope="scope">
              {{scope.row.articleTitle||'--'}}
            </template>
          </el-table-column>
          <el-table-column label="来源" prop="articleOrigin">
            <template slot-scope="scope">
              {{scope.row.articleOrigin||'--'}}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    search() {//获取设备列表
      this.loading = false;
      var self = this;
      // self.$http.post('/article/getHtmlarticleList', {startPosition:((this.params.pageNo-1) * this.params.pageSize),maxLength:this.params.pageSize}, result => {
      //   if (result.code) {
      //     console.log(res);
    ...
    searchBot(page){
      ...
      self.$http.post('/noticeMng/queryNoticeInfoByCondition',{pageStart:((this.params.botNo-1) * this.params.botSize) ,pageSize:this.params.botSize,articleTitle:this.articleTitle||''},result=>{
        // console.log(result);
          if (result.code) {
    ...
    delItem(articleCode) {
      var self = this;
      self.$http.post('/noticeMng/deleteNoticeInfo', { "articleCode": articleCode }, result => {
        if (result.code) {
          self.$message({
            message: result.message,
            type: 'success'
