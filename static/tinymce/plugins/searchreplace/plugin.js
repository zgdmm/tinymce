...
var searchreplace = (function () {
  ...
  var $_307ypdjpjivb5ecb = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_307ypdjpjivb5ecb.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_nw8wyjnjivb5ec6.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_nw8wyjnjivb5ec6.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_nw8wyjnjivb5ec6.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_nw8wyjnjivb5ec6.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_nw8wyjnjivb5ec6.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_nw8wyjnjivb5ec6.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_nw8wyjnjivb5ec6.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_nw8wyjnjivb5ec6.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_nw8wyjnjivb5ec6.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_nw8wyjnjivb5ec6.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_nw8wyjnjivb5ec6.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_nw8wyjnjivb5ec6.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_nw8wyjnjivb5ec6.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_nw8wyjnjivb5ec6.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_nw8wyjnjivb5ec6.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_eth65hjrjivb5eck = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_eth65hjrjivb5eck.open(editor, currentIndexState);
    });
  };
  var $_ad15i2jqjivb5ech = { register: register };
  ...
  var $_3504ukjsjivb5ecn = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_ad15i2jqjivb5ech.register(editor, currentIndexState);
    $_3504ukjsjivb5ecn.register(editor, currentIndexState);
    return $_gg468ejmjivb5ec2.get(editor, currentIndexState);
  });
  function Plugin () {
  }
